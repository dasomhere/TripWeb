...
<section class="container">
	...
	<ul data-ng-if="searchDetailIntro[0].contenttypeid == 12 ">
		...
		<li data-ng-repeat="intro in searchDetailIntro">체크카드 사용 여 부:
			{{intro.chkcreditcard}}</li>
		<li data-ng-repeat="intro in searchDetailIntro">애완동물 출입가능여부:
			{{intro.chkpet}}</li>
		<li data-ng-repeat="intro in searchDetailIntro"
		...
		<li data-ng-repeat="intro in searchDetailIntro"
			data-ng-if="intro.restdate != null">쉬 는 날 : {{intro.restdate}}</li>
		<li data-ng-repeat="intro in searchDetailIntro"
			data-ng-if="intro.usetime == null">이 용 시 간 : 24시간</li>
		<li data-ng-repeat="intro in searchDetailIntro"
			data-ng-if="intro.usetime!= null">이 용 시 간 : {{intro.usetime}}</li>
		<li data-ng-repeat="common in searchDetailIntro"
	...
	<ul data-ng-if="searchDetailIntro[0].contenttypeid == 15 ">
		...
		<li data-ng-repeat="event in searchDetailIntro"
			data-ng-if="event.agelimit == null"><b>연 령 : </b> 모든 연령</li>
		<li data-ng-repeat="event in searchDetailIntro"
			data-ng-if="event.agelimit != null"><b>연 령 : </b>
			{{event.agelimit}}</li>

		<li data-ng-repeat="event in searchDetailIntro"><b>기 간 : </b>
			{{event.eventstartdate}} - {{event.eventenddate}}</li>

		<li data-ng-repeat="event in searchDetailIntro"><b>전 화 번 호 :
		</b> {{event.sponsor1tel}}</li>
		...
		<li data-ng-repeat="event in searchDetailIntro"
			data-ng-if="event.placeinfo != ''"><b>오 시 는 길 : </b> <span
	...
	<ul data-ng-if="searchDetailIntro[0].contenttypeid == 15 ">
		<li data-ng-repeat="event in searchDetailInfo">
